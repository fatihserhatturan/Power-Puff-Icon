// This file is auto-generated. Do not edit manually.
// Source: svgs/social/cannabis.svg
import { forwardRef } from 'react'
import type { IconMeta } from '@beluga-icon/core'
import { Icon } from '../../components/Icon'
import type { IconProps } from '../../components/Icon'

export const CannabisMeta: IconMeta = {
  name: 'cannabis',
  category: 'social',
  tags: [],
  version: '0.1.0',
}

export const CannabisIcon = forwardRef<SVGSVGElement, IconProps>(function CannabisIcon(props, ref) {
  return (
    <Icon ref={ref} {...props}>
      <path d="M12 22v-4" />
      <path d="M7 12c-1.5 0-4.5 1.5-5 3 3.5 1.5 6 1 6 1-1.5 1.5-2 3.5-2 5 2.5 0 4.5-1.5 6-3 1.5 1.5 3.5 3 6 3 0-1.5-.5-3.5-2-5 0 0 2.5.5 6-1-.5-1.5-3.5-3-5-3 1.5-1 4-4 4-6-2.5 0-5.5 1.5-7 3 0-2.5-.5-5-2-7-1.5 2-2 4.5-2 7-1.5-1.5-4.5-3-7-3 0 2 2.5 5 4 6" />
    </Icon>
  )
})

CannabisIcon.displayName = 'CannabisIcon'
