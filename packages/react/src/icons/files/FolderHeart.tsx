// This file is auto-generated. Do not edit manually.
// Source: svgs/files/folder-heart.svg
import { forwardRef } from 'react'
import type { IconMeta } from '@beluga-icon/core'
import { Icon } from '../../components/Icon'
import type { IconProps } from '../../components/Icon'

export const FolderHeartMeta: IconMeta = {
  name: 'folder-heart',
  category: 'files',
  tags: [],
  version: '0.1.0',
}

export const FolderHeartIcon = forwardRef<SVGSVGElement, IconProps>(
  function FolderHeartIcon(props, ref) {
    return (
      <Icon ref={ref} {...props}>
        <path d="M10.638 20H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h3.9a2 2 0 0 1 1.69.9l.81 1.2a2 2 0 0 0 1.67.9H20a2 2 0 0 1 2 2v3.417" />
        <path d="M14.62 18.8A2.25 2.25 0 1 1 18 15.836a2.25 2.25 0 1 1 3.38 2.966l-2.626 2.856a.998.998 0 0 1-1.507 0z" />
      </Icon>
    )
  },
)

FolderHeartIcon.displayName = 'FolderHeartIcon'
