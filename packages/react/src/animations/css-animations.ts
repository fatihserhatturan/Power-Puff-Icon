...
const CSS_PRESETS: Record<string, CssPreset> = {
  ...
  heartbeat, flash, tada, jello, swing,
  rubberBand, flipX, breathe,
  neon, glitch, wobble, roll, zoomIn, fadeUp,
...
export const SPEED_DURATION: Record<string, Record<string, string>> =
  Object.fromEntries(
...
const COMBINED_CSS =
  Object.values(CSS_PRESETS).map(p => p.css).join('\n') +
